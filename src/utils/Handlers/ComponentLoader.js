...
// Map des noms de modules vers leurs clés dans le client
const moduleKeys = {
    'commands': 'commands',
...
module.exports = function (client, folder = null) {
    ...
    for (const [module, moduleKey] of Object.entries(moduleKeys)) {
        if (typeof folder === 'string' && folder !== module) continue;

        client[moduleKey] = new Map();
            ...
            try {
                ...
                switch (moduleKey) {
                    case 'messages':
                        ...
                        addComponent(client[moduleKey], data.name, data);
                        for (const alias of data.aliases) {
                            addComponent(client[moduleKey], alias, data);
                        }
                        break;
                    case 'commands':
                    ...
                    case 'modals':
                        ...
                        addComponent(client[moduleKey], data.customID, data);
                        break;
                }
            } catch (error) {
        ...
        client.logs.info(`Loaded ${client[moduleKey].size} ${module}`);
    }
};
