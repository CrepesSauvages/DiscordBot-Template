...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Récupérer les cas de modération depuis l'API du bot
    const response = await axios.get(`${BOT_API_URL}/guilds/${id}/moderation`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
    
    // Répondre avec les cas de modération
    return res.status(200).json(response.data);
  } catch (error) {
    console.error(`Erreur lors de la récupération des cas de modération pour le serveur ${id}:`, error);
